...
docker-webhook : build
	...
	docker build --no-cache -t khulnasoft/postee:latest -f Dockerfile.release .
	docker run -p 8082:8082 -p 8445:8445 khulnasoft/postee:latest --cfgfile /server/cfg.yaml

docker-ui :
	@echo "Building image Dockerfile.ui...."
	docker build --no-cache -t khulnasoft/postee-ui:latest -f Dockerfile.ui .
